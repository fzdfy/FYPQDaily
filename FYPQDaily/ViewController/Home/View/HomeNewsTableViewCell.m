...
@implementation CellNewsView
- (id)initWithFrame:(CGRect)frame style:(NewsCellLayoutStyle)style{
    if(self = [super initWithFrame:frame]){
        switch (style) {
                case NewsCellLayoutStyleAbove:    //cellType = 0,图片在上，文字在下
                [self _aboveStyle];
                break;
                case NewsCellLayoutStyleRight:    //cellType = 1,图片在右，文字在左
                [self _rightStyle];
                break;
...
- (void)_aboveStyle {
    ...
    _newsTitleLabel.textColor = RGBA(42, 42, 42, 0.99);
...
- (void)_layout {
    if (_layout.style == NewsCellLayoutStyleAbove) {          //cellType = 0,图片在上，文字在下
        [self _newsView];
    }else if (_layout.style == NewsCellLayoutStyleRight) {    //cellType = 1,图片在右，文字在左
        [self _newsView];
        [self _bottomViewWithWidth:self.frame.size.width / 2];
    }else if (_layout.style == NewsCellLayoutStyleDetails) {  //cellType = 2,图片在上，文字在下，下方有“评论”和“喜欢”数值
        [self _newsView];
...
- (void)_newsView {
    ...
    _newsView = [[CellNewsView alloc] initWithFrame:CGRectMake(0, 5, w, _layout.height - 5) style:_layout.style];
    [_cellBackgroundView addSubview:_newsView];
}

- (void)_bottomViewWithWidth:(CGFloat)width {
    _bottomView = [[CellBottomView alloc] initWithFrame:CGRectMake(0, _layout.height-5-21, width, 21)];
...
- (void)setLayout:(NewsCellLayout *)layout {
    ...
    cellSeparator.backgroundColor = RGBA(238, 238, 238, 1);
